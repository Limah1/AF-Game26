...
func _populate_options() -> void:
	for label in ["Normal", "Chuva", "Neve"]:
...
func _is_weather_outfit() -> bool:
	return outfit_option.selected > 0
...
func _on_Save_pressed() -> void:
	var error = ResourceSaver.save(TUNING_PATH, tuning)
	if error == OK:
